...
We separately examined whether context could reduce cross-species error on the frozen panel using a context-aware species-transfer calibration (`phylo_organ_gate_v1`). The gate uses organ and phylogenetic support estimated from source species: it retains expression-based transfer when informative same-family support is available, and otherwise falls back to an organ prior. On the same 3,964 cells, exact-label denominator and 55.90% source-label coverage, all-cell accuracy rises from 23.64% for the centroid baseline through 30.10% for expression STC and 31.84% for neural STC to 42.36% for the context-aware gate (Fig. 2e). Covered-label accuracy reaches 75.77% and covered-label macro-F1 is 0.3045. Because configuration selection aggregates outer-fold information, this is a global context-sensitivity analysis rather than a replacement for the nested v17 primary result. It is retained to identify an implementable context module and to make its selection boundary inspectable.
