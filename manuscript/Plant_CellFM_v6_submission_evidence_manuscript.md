...
On the same locked wheat-root test, an official scPlantLLM checkpoint loaded without state-key discrepancies reached 0.2001 macro-F1 through a frozen encoder plus train-only centroid readout and 0.2998 macro-F1 after final-block-plus-new-head partial adaptation, with an independently replayed prediction record. This reference shares the prepared object, author mapping and test barcodes but is not presented as a full-backbone or compute-budget-matched rank.
...
Seurat label transfer and a cosine-centroid baseline have separate recorded executions. We additionally evaluated the official scPlantLLM checkpoint on the GSE270342 prepared object through the exact author first-target mapping and the exact 1,433-cell locked Plant-CellFM test barcode set (Extended Data Fig. 8; Tables S22-S23). The checkpoint loaded on CUDA with zero missing or unexpected state keys. A frozen-encoder, train-only centroid readout obtained 0.2107 accuracy and 0.2001 macro-F1. We then froze the first five transformer blocks and trained only the final block plus a new 13-class head, selecting epoch 4 solely by validation macro-F1; this partial adaptation reached 0.3426 accuracy and 0.2998 macro-F1 on the locked test. A separate replay audit regenerated the test predictions and metrics from the recorded adapter checksum. The shared object, mapping and lock make this an informative executable reference, but different trainable scope and compute mean it is neither full-backbone scPlantLLM fine-tuning nor a universal model ranking. scPlantAnnotate still requires authenticated execution or an official prediction export.
...
### Supervised secondary-root and wheat-root adapters
...
The strict-transfer, secondary-root and matched wheat-root figures use fixed-seed nonparametric bootstrap resampling only to visualize uncertainty conditional on their frozen test populations. Every quantitative visual panel has a TSV source-data file. All v6 main and Extended Data figures are exported as editable SVG and PDF, PNG preview and 600-dpi TIFF. The v6 audit checks file presence, editable SVG text, source-table presence, raster resolution, frozen strict metrics, both released adapter checksums, the wheat overlap exclusion and fixed split, and the scPlantLLM official-checkpoint, partial-adapter checksum and prediction-replay contracts. It reports technical readiness but does not substitute a subjective readiness score for editorial or peer review.

## Data and Code Availability

The repository, source code, v6 manuscript, model card, evidence ledger, figures, source data and reproducibility records are available at https://github.com/ahvsjags/SnowLotus-CellFM on branch `agent/remote-pipeline-20260728`. The GSE270140 and GSE270342 adapters are tracked through Git LFS and have SHA256 `1a306c4a5e21630a75a5a63d2867e86712b2da78eea3805eb5dc00b957134fd7` and `597b7e425b0355ddfa81e4f5c9c63e85987d6b72fe49a841736b200ea6c2c22e`, respectively. Reproduce the context-adaptation and matched-reference evidence with:
...
python scripts/render_v6_main_figures.py
...
**Figure 5 | Provenance-controlled allopolyploid wheat-root adaptation.** The author-labelled GSE270342 wheat object is shown after exclusion of 224 exact CS1 barcodes overlapping a historical exploratory strict-transfer record. The frozen first-target diagnostic, full 13-class locked-test confusion matrix, per-class F1 and validation-only model selection are presented separately. The adapter is trained and evaluated within the same public author-labelled study on a fixed cell-level split; it is neither a zero-shot result nor independent external validation.
...
**Extended Data Figure 8 | Matched official scPlantLLM reference.** The official scPlantLLM checkpoint uses the same GSE270342 prepared object, author first-target mapping and 1,433 locked test barcodes as the Plant-CellFM wheat adapter. The frozen centroid reference and final-block-plus-new-head partial adaptation are shown with per-class changes and a replay audit. This establishes a matched executable reference but not full-backbone fine-tuning, compute-matched comparison or a universal ranking.
