    ...
    if attempt_count:
        if ssh.get("ok") is True:
            audit_line = (
                ...
                f"The latest direct px2 audit observed TCP probe ok=`{tcp.get('ok')}` "
                f"and SSH probe ok=`{ssh.get('ok')}`; the legacy watcher log is retained "
                f"only for provenance at attempt `{attempt_count}/240`."
            )
        else:
            audit_line = (
                "- Local recovery status is generated by `scripts/write_ssh_recovery_status.py`; "
