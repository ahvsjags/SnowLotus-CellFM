...
"""Run a matched full-backbone scPlantLLM fine-tuning baseline on wheat.
...
loaded cleanly, a new 13-class head is fitted and every backbone parameter is
trainable. Epoch selection reads validation macro-F1 only; the locked test is
evaluated once after the selected full model is restored.
"""
...
OUTPUT_DIR = ROOT / "outputs" / "external_benchmarks" / "scplantllm_gse270342_full_finetune"
OUTPUT_JSON = ROOT / "release_metadata" / "scplantllm_gse270342_full_finetune_v1.json"
OUTPUT_TABLE = ROOT / "supplementary_tables" / "submission_v4" / "Supplementary_Table_S24_scPlantLLM_GSE270342_full_finetune.tsv"
...
def configure_full_backbone_adaptation(model: torch.nn.Module, head: torch.nn.Module) -> tuple[list[str], int]:
    trainable = []
    ...
    for parameter in head.parameters():
        parameter.requires_grad_(True)
    return trainable, int(sum(parameter.numel() for parameter in model.parameters()) + sum(parameter.numel() for parameter in head.parameters()))
...
def snapshot_full_model(model: torch.nn.Module, head: torch.nn.Module) -> dict[str, Any]:
    return {
        "full_backbone": {name: tensor.detach().cpu().clone() for name, tensor in model.state_dict().items()},
        "classification_head": {name: tensor.detach().cpu().clone() for name, tensor in head.state_dict().items()},
    }


def restore_full_model(model: torch.nn.Module, head: torch.nn.Module, state: dict[str, Any]) -> None:
    model.load_state_dict(state["full_backbone"], strict=True)
...
def main() -> int:
    ...
    parser.add_argument("--unweighted-loss", action="store_true", help="Use ordinary cross entropy instead of the default train-only class-balanced loss.")
    parser.add_argument("--publish", action="store_true", help="Write the formal release record only for an unrestricted complete run.")
    ...
    has_debug_limits = any(value is not None for value in (args.limit_train, args.limit_validation, args.limit_test))
    if args.publish and has_debug_limits:
        raise ValueError("--publish cannot be combined with debug limits.")
    release_mode = args.publish and not has_debug_limits
    ...
    if missing_keys or unexpected_keys:
        raise ValueError("Official scPlantLLM checkpoint did not load cleanly before full-backbone adaptation.")
    ...
    trainable_names, trainable_parameters = configure_full_backbone_adaptation(model, head)
    ...
    train_class_counts = np.bincount(train_targets, minlength=len(label_names)).astype(np.float64)
    class_weights = train_targets.size / (len(label_names) * train_class_counts)
    criterion = torch.nn.CrossEntropyLoss(
        weight=None if args.unweighted_loss else torch.as_tensor(class_weights, dtype=torch.float32, device=device)
    )
    ...
    adapter_path = output_dir / "best_full_backbone_finetune.pt"
    torch.save(
        {
            "schema_version": "scplantllm_full_backbone_wheat_finetune_v1",
            "base_checkpoint_sha256": sha256(CHECKPOINT),
            "label_names": label_names.tolist(),
            "best_epoch": best_epoch,
            "best_validation_macro_f1": best_validation_macro_f1,
            "full_finetune_state": best_state,
        },
        adapter_path,
    )
    ...
    predictions = pd.DataFrame({"cell_id": cell_ids[test_indices], "author_label": labels[test_indices], "scplantllm_full_finetune_prediction": test_predictions})
    ...
    record = {
        "schema_version": "plant_cellfm_scplantllm_gse270342_full_finetune_v1",
        "status": "COMPLETED_MATCHED_FULL_BACKBONE_ADAPTATION" if release_mode else "DEBUG_NOT_RELEASE_ELIGIBLE",
        "claim_boundary": (
            "This is a matched-data full-backbone scPlantLLM adaptation baseline on the GSE270342 wheat stress test. "
            "It uses the same author object, first-target orthogroup mapping and exact released Plant-CellFM locked test barcodes. "
            "Every scPlantLLM backbone parameter and a new 13-class head are trainable. "
            "It is a same-study adaptation reference, not a strict leave-species result, independent external validation, compute-budget-matched comparison or universal model ranking."
        ),
        "input_contract": {
            "dataset": "GSE270342",
        ...
        "model": {
            ...
            "adaptation": {"mode": "new_13_class_head_plus_full_scplantllm_backbone", "trainable_backbone_parameter_names": trainable_names, "trainable_parameter_count": trainable_parameters, "frozen_backbone_layers": []},
        },
        ...
        "optimization": {"optimizer": "AdamW", "backbone_lr": args.backbone_lr, "head_lr": args.head_lr, "weight_decay": args.weight_decay, "grad_clip": args.grad_clip, "loss": "cross_entropy" if args.unweighted_loss else "train_only_class_balanced_cross_entropy", "train_class_counts": {str(label): int(train_class_counts[index]) for index, label in enumerate(label_names)}, "class_weights": None if args.unweighted_loss else {str(label): float(class_weights[index]) for index, label in enumerate(label_names)}},
        ...
        "artifacts": {"full_finetune_checkpoint": adapter_path.relative_to(ROOT).as_posix(), "full_finetune_checkpoint_sha256": sha256(adapter_path), "validation_history": (output_dir / "validation_history.tsv").relative_to(ROOT).as_posix(), "locked_test_predictions": (output_dir / "locked_test_predictions.tsv").relative_to(ROOT).as_posix(), "locked_test_per_class": (output_dir / "locked_test_per_class.tsv").relative_to(ROOT).as_posix()},
    }
    if release_mode:
