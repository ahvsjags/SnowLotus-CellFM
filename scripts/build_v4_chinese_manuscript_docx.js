...
/* Build a clean Chinese submission manuscript from the evidence-first v4 Markdown. */
...
function main() {
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 250 },
    children: [new TextRun({ text: "协议感知跨物种注释与目标物种适配 | 证据优先主文 v4", font, size: 19, color: muted, italics: true })],
  }));
  ...
  const figures = [
    ["plant_cellfm_v4_fig1_cross_species_atlas", "图 1 | 跨物种植物单细胞图谱的可追溯语料与共享表征。", 480],
    ["plant_cellfm_v4_fig2_nested_strict_transfer", "图 2 | 嵌套严格留物种迁移与标签完整性。", 480],
    ["plant_cellfm_v4_fig3_fewshot_target_adaptation", "图 3 | 少样本目标物种适配的剂量响应。", 480],
  ...
  const document = new Document({
    ...
    sections: [{
      ...
      footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Plant-CellFM v4 | 证据优先中文主文", font, size: 15, color: muted })] })] }) },
      children,
    }],
  });
