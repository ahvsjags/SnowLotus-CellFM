...
"""Replay and audit the matched partial scPlantLLM wheat adaptation baseline."""
...
RECORD_PATH = ROOT / "release_metadata" / "scplantllm_gse270342_partial_finetune_v1.json"
OUTPUT_PATH = ROOT / "release_metadata" / "scplantllm_gse270342_partial_finetune_audit_v1.json"
...
def audit(device: str = "cuda", batch_size: int = 8) -> dict[str, Any]:
    record = json.loads(RECORD_PATH.read_text(encoding="utf-8"))
    if record["status"] != "COMPLETED_MATCHED_PARTIAL_BACKBONE_ADAPTATION":
        raise ValueError("Partial fine-tune record is not release eligible.")
    adapter_path = ROOT / record["artifacts"]["adapter_checkpoint"]
    prediction_path = ROOT / record["artifacts"]["locked_test_predictions"]
    if not adapter_path.is_file() or not prediction_path.is_file():
        raise FileNotFoundError("Partial scPlantLLM adapter checkpoint or prediction artifact is missing.")
    if sha256(adapter_path) != record["artifacts"]["adapter_checkpoint_sha256"]:
        raise ValueError("Partial scPlantLLM adapter checkpoint checksum does not match the release record.")
    ...
    last_layer = int(adapter["last_layer"])
    head = finetune.make_head(shape.d_model, len(label_names)).to(torch_device)
    finetune.configure_partial_adaptation(model, head, last_layer=last_layer)
    finetune.restore_adapter(model, head, adapter["adapter_state"])
    ...
    replay = pd.DataFrame(
        {
            ...
            "scplantllm_partial_finetune_prediction": label_names[np.concatenate(predictions)],
        }
    )
    ...
    return {
        "schema_version": "plant_cellfm_scplantllm_gse270342_partial_finetune_audit_v1",
        ...
        "replay": {
            ...
            "adapter_checkpoint_sha256": sha256(adapter_path),
