...
| P2 | Rebuild species-holdout with clearer label harmonization and class ontology mapping. | Coverage audit and embedding-based ontology-label benchmark are completed in `release_metadata/species_ontology_coverage_audit_v9.md` and `release_metadata/species_ontology_label_benchmark_v9.md`; next step is model-side improvement, not metric bookkeeping. |
...
Plant-CellFM v9 is currently best described as a reproducible plant-general foundation-model and adapter framework for single-cell expression annotation. Its strongest evidence is the complete engineering chain: public corpus, frozen RTX 4090 checkpoint, strict cross-group benchmark, v3/centroid/Seurat comparisons, species-holdout failure audit, ontology coverage audit, ontology-label benchmark, figure-ready Arabidopsis root biological case, GitHub release asset, editor package and live CUDA service. The main limitation for top-tier review is not the existence of the system but the remaining need for stronger independent external-model closure, model-side cross-species transfer improvement and broader biological replication.
