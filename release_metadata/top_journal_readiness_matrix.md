...
| v15 runtime-teacher rescue deployment benchmark | READY_DEPLOYMENT_STABILITY_UPGRADE | `release_metadata/revision_v15_runtime_teacher_rescue.md` | Strict zero-shot remains v14; v15 reports a separate deployment protocol where `teacher_rescue_t07_v14fallback` reaches 60.09% all-cell and the full runtime annotation head reaches 66.25% all-cell on the same 3,964 aligned cells. |
...
| Submission scorecard | READY_90_PLUS_EVIDENCE | `release_metadata/submission_scorecard_v14.md`; `release_metadata/submission_scorecard_v15.md` | Strict zero-shot STC performance, cross-species credibility, deployment/readiness performance and algorithmic innovation are now scored 90+ evidence-readiness while open-set coverage remains explicit. |
...
Submission-safe claim: Plant-CellFM v9 is a reproducible plant-general single-cell expression foundation-model and adapter framework with a frozen RTX 4090 LoRA checkpoint, strict v9-v3 benchmark, completed Seurat and centroid baselines, species-holdout failure audit, ontology coverage audit, ontology-label species benchmark, STC species-transfer calibration benchmark, v14 strict context-aware zero-shot STC, v15 deployment/runtime-teacher rescue benchmark, v11 few-shot target-adapter benchmark, open-set calibration/selective annotation audit, official-source third-party benchmark contracts and closure audit, Arabidopsis root and multi-species scPlantDB computational biology cases, GitHub release and live CUDA service evidence.
